---
id: first-palaces
name: The first Minoan palaces
type: event
subtype: process
tint: minoan
start: -1900
end: -1700
approx: true
coords:
  - 35.298
  - 25.163
claims:
  - text: Palatial complexes appear on Crete from c. 1900 BC.
    evidence: archaeological
    confidence: established
  - text: They combined storage, craft production, administration and ritual functions.
    evidence: archaeological
    confidence: established
  - text: Administrative record-keeping using written scripts accompanied their emergence.
    evidence: epigraphic
    confidence: established
  - text: The first palaces were destroyed c. 1700 BC by earthquake.
    evidence: debate
    confidence: probable
relations:
  - id: minoan-civilisation
    rel: began
  - id: knossos
    rel: site
  - id: phaistos
    rel: site
  - id: linear-a
    rel: accompanied by
sources:
  - cline2010
  - macdonald2005
_order: 3
---

<!-- field: summary -->
Around 1900 BC large multi-functional complexes appear at Knossos, Phaistos, Malia and Zakros — the first states in Europe.

<!-- field: body -->
The First Palace period marks a step change: centralised storage on an industrial scale, craft workshops, and administration recorded in Cretan Hieroglyphic and then Linear A. Storage capacity at Knossos alone ran to hundreds of thousands of litres.

Why this happened on Crete and not the mainland is debated. Crete escaped the destructions of c. 2200 BC, sits at the crossing point of Egyptian, Levantine and Aegean routes, and produced surplus oil and wine that reward central storage and redistribution.

All the first palaces were destroyed around 1700 BC, probably by earthquake, and immediately rebuilt on a grander scale.
