---
id: euclid
name: Euclid
altNames:
  - Εὐκλείδης
type: person
subtype: mathematician
tint: hellenistic
start: -325
end: -265
approx: true
floruit: true
coords:
  - 31.2
  - 29.92
region: Alexandria
claims:
  - text: The Elements systematises Greek geometry from a small set of postulates.
    evidence: literary
    confidence: established
  - text: It remained a standard mathematics textbook into the 20th century.
    evidence: consensus
    confidence: established
  - text: The parallel postulate is independent of the other four.
    evidence: consensus
    confidence: established
  - text: Euclid was a single individual rather than a group of authors.
    evidence: debate
    confidence: debated
relations:
  - id: elements-euclid
    rel: wrote
  - id: alexandria
    rel: worked at
  - id: archimedes
    rel: preceded
sources:
  - heath1908
  - netz1999
_order: 39
---

<!-- field: summary -->
Author of the Elements, the most influential textbook ever written and the model of deductive reasoning for two thousand years.

<!-- field: significance -->
Established the axiomatic method: a handful of definitions and postulates, from which everything else is proved.

<!-- field: body -->
Almost nothing is known about Euclid's life. The *Elements* organises the mathematics of his predecessors into thirteen books proceeding from five postulates. It was used as a school text into the twentieth century — only the Bible has been printed in more editions.

Book IX contains the proof that there are infinitely many primes, still a model of elegant argument. The fifth postulate, about parallel lines, is noticeably less self-evident than the others; attempts to derive it from the rest failed for two millennia, and in the nineteenth century Lobachevsky, Bolyai and Riemann showed why — dropping it produces consistent non-Euclidean geometries, which turned out to describe physical spacetime.

Asked by Ptolemy for a shorter route to geometry, he is said to have replied that there is no royal road.
