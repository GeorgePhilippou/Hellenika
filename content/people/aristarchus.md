---
id: aristarchus
name: Aristarchus of Samos
altNames:
  - Ἀρίσταρχος
type: person
subtype: astronomer
tint: hellenistic
start: -310
end: -230
approx: true
coords:
  - 37.75
  - 26.98
region: Samos; Alexandria
claims:
  - text: Aristarchus proposed that the Earth orbits the Sun.
    evidence: literary
    confidence: strong
  - text: His heliocentric argument is known chiefly through Archimedes' summary.
    evidence: literary
    confidence: established
  - text: The absence of observable stellar parallax was a principal ancient objection to heliocentrism.
    evidence: consensus
    confidence: strong
  - text: His treatise on the sizes and distances of the Sun and Moon survives.
    evidence: literary
    confidence: established
  - text: A crater on the Moon is named Aristarchus in his honour.
    evidence: consensus
    confidence: established
relations:
  - id: archimedes
    rel: reported by
  - id: alexandria
    rel: worked at
  - id: hipparchus
    rel: preceded
sources:
  - heath1913
  - shipley2000
_order: 41
---

<!-- field: summary -->
The first person known to have placed the Sun, not the Earth, at the centre of the cosmos — eighteen centuries before Copernicus.

<!-- field: significance -->
Proposed heliocentrism with a coherent supporting argument, and was set aside for reasons that were, on the evidence then available, defensible.

<!-- field: body -->
Aristarchus' heliocentric proposal survives only because Archimedes summarised it in *The Sand Reckoner*. He also argued that the sphere of fixed stars was immensely larger than previously supposed — a necessary consequence, since no stellar parallax could be observed.

That was precisely the objection. If the Earth moves, nearby stars should shift against distant ones over the year. No such shift was detectable, so either the stars were unimaginably far away or the Earth was still. Ancient astronomers reasonably chose the second. Stellar parallax was not measured until 1838.

His surviving treatise on the sizes and distances of the Sun and Moon uses sound geometry with imprecise measurements, and gets the Sun about twenty times too close — but establishes correctly that it is much larger than the Earth.

His later recognition as a precursor to Copernicus is commemorated in the sky itself: one of the brightest and most prominent craters on the Moon, a plateau region marked by unusually reflective material, bears the name Aristarchus, a small but lasting acknowledgment of a proposal that had to wait roughly eighteen centuries before it was taken up again.
